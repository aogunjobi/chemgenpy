...
# Add the parent directory of 'chemgenpy' to sys.path
...
def config_builder():
    display(widgets.HTML(value="<font color=blue><font size=5><b><u>CHEMGENPY</font>"))
    global input_direc
...
def building_blocks():
    ...
    type_smiles_intro = widgets.HTML(
        """<font size=3>Create a building blocks file by entering individual/comma separated SMILES of each building block.""",
        ...
        try:
            ...
            if os.stat(file_name).st_size == 0:
                        ...
                        try:
                            ...
                            if mol:
                                correct = widgets.HTML("""<font size=3>The file has been uploaded successfully""",
                                                       layout=widgets.Layout(height='85px', width='90%', size='20'))
                                smiles.append(line.strip())
                            else:
                                incorrect = widgets.HTML(line + " in file is an incorrect SMILES")  # checking the SMILES in the building blocks file
                                display(incorrect)
                        except Exception as e:
                            incorrect = widgets.HTML(line + " in file caused an error: " + str(e))
                            display(incorrect)
                ...
                if len(smiles) > 0:
                    ...
                    ibu1 = Draw.MolsToGridImage(mol_lists)
                    display(ibu1)
                    generation_rules()
    ...
    def on_another_clicked(i):
        ...
        for element in f:
            try:
                ...
                print("Error processing SMILES " + element + ": " + str(e))
    ...
    def on_visualization_clicked(t):
        ...
        try:
            ...
            ibu = Draw.MolsToGridImage(mol_list)
            display(ibu)
        except Exception as e:
            print("You typed a wrong SMILES. Unable to visualize. Error: " + str(e))
    ...
    def second_section(q):
        if os.stat("building_blocks.dat").st_size <= 0:
            ...
            building_blocks = widgets.Text(value='None',
                                           ...
                                           style=style)
            bb_intro = widgets.HTML(
                """Specify the building blocks which must be present in all the molecules in the final library""",
            ...
            left_box = widgets.VBox(
                [Label(''), Label('Bonds'), Label('Atoms'), Label('Molecular Weight'), Label('Rings'), Label('Aromatic Rings'), Label('Non Aromatic Rings'), Label('Single Bonds'), Label('Double Bonds'), Label('Triple Bonds)')])
            ...
            satom_intro = widgets.HTML(
                """Specify the maximum number of heteroatoms that must be present in the  molecules in the final library.Expected type: tuple of tuple(s) (('Cl', 10), )""",
                layout=widgets.Layout(height='45px', width='90%', size='20'))
            ...
            substructure = widgets.Text(
                ...
                style=style)
            sub_intro = widgets.HTML(
                """Enter substructures in SMARTS format which must be included in all molecules in the final library
            """,
                layout=widgets.Layout(height='45px', width='90%', size='20'))
            ...
            substructure_exclusion = widgets.Text(
                ...
                style=style)
            sube_intro = widgets.HTML(
                """Enter substructures in SMARTS format which must be excluded in all molecules in the final library
            """,
                layout=widgets.Layout(height='45px', width='90%', size='20'))
            ...
            ibb_intro = widgets.HTML(
                """Should the initial building blocks be included in the final library""",
                layout=widgets.Layout(height='45px', width='90%', size='20'))
        ...
        def command_line_arguments(b):
            ...
            comb_intro = widgets.HTML("""Select the combination type for generating the molecules""",
                                      layout=widgets.Layout(height='50px', width='90%', size='20'))
            ...
            def on_button_clicked(b):
                ...
                envi_intro = widgets.HTML("""Specify the virtual environment in which you want to run the library generator""",
                                          layout=widgets.Layout(height='50px', width='90%', size='20'))
                envi = widgets.Text(
                    value='None',
                ...
                run = widgets.Button(description='Run Chemgenpy', layout=Layout(width='auto', border='solid 1px black'), style=style)
                            ...
                            if GA_func.value == "":
                                ...
                                try:
                                    ...
                                    fitness = widgets.Text(
                                        ...
                                        style=style)
                                    fitness_intro = widgets.HTML(
                                        """Enter the fitness values. Expected type: tuple of tuple(s): (("max",0.7),)
                                    """, layout=widgets.Layout(height='45px', width='90%', size='20'))
                ...
                interact(constraints, x=widgets.RadioButtons(options=['with constraints', 'without constraints'], value='without constraints', description='Run ChemGenPy ', disabled=False))
                display(interact)

            def use_ga(j):
                from chemgenpy.constrained import GeneticAlgorithm  # import genetic algorithm
            ...
            def on_run_clicked(c):
                if envi.value == "":
                    envi.value == 'None'
                    if directory.value == "":
                        os.system("chemgenpyshell -i config.dat -b building_blocks.dat -o ./output/")
                    else:
                        os.system("chemgenpyshell -i config.dat -b building_blocks.dat -o " + directory.value)
                else:
                    try:
                    ...
                    if directory.value == "":
                        os.system("chemgenpyshell -i config.dat -b building_blocks.dat -o ./")
                    else:
                        os.system("chemgenpyshell -i config.dat -b building_blocks.dat -o " + directory.value)
            ...
            def on_stats_clicked(d):
                ...
                feasibility_intro = widgets.HTML(
                    """ Test the synthetic feasibility of the generated library molecules
                                    """, layout=widgets.Layout(height='45px', width='90%', size='20'))
